...
При звичайній реєстрації через ендпоінт API створюється користувач із базовими правами клієнта.

Для доступу до захищених функцій управління салоном потрібно змінити роль безпосередньо в базі даних.

Підключіться до вашої бази PostgreSQL (наприклад, через pgAdmin або DBeaver) і виконайте цей SQL-запит:

```sql
UPDATE "Users"
SET "Role" = 'Admin'
WHERE "Username" = 'тут_логін_вашого_користувача';
```
